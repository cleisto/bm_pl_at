...
function ZoomWrap({ children }) {
  var _z = useState(function() { return { scale: 1, cx: 360, cy: 190 }; }); var zm = _z[0]; var setZm = _z[1];
  var containerRef = useRef(null);
  function getOrigVB() {
    if (!containerRef.current) return { w: 720, h: 380 };
    var svg = containerRef.current.querySelector('svg');
    var vb = svg ? (svg.getAttribute('data-orig-vb') || svg.getAttribute('viewBox')) : null;
    if (vb) { var p = vb.split(' ').map(Number); return { w: p[2], h: p[3] }; }
    return { w: 720, h: 380 };
  }
  ...
  function handleTM(e) {
    ...
    if (g.pinching && e.touches.length === 2) {
      ...
      var dxSvg = (mx - g.midX0) / rect.width * (vb.w / g.scale0);
      var dySvg = (my - g.midY0) / rect.height * (vb.h / g.scale0);
      ...
      var vb2 = getOrigVB();
      var dx = (e.touches[0].clientX - g.px0) / rect2.width * (vb2.w / zm.scale);
      var dy = (e.touches[0].clientY - g.py0) / rect2.height * (vb2.h / zm.scale);
  ...
  function reset() { var vb = getOrigVB(); setZm({ scale: 1, cx: vb.w / 2, cy: vb.h / 2 }); }
